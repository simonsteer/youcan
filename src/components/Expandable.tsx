...
export interface ExpandableProps {
  children: (renderProps: { setIsOpen: Dispatch<SetStateAction<boolean>>, isOpen: boolean;  setHeight: Dispatch<SetStateAction<number>>, height: number }) => React.ReactNode
  closedHeight?: number
  showArrow?: boolean
  absolute?: boolean
  closeOnBlur?: boolean
...
const Expandable = ({ children, closedHeight = 0, startOpen = false, showArrow = false, closeOnBlur = false }: ExpandableProps) => {
  ...
  return (
    <ExpandableContainer ref={root} hasOpened={hasOpened} height={height} closedHeight={closedHeight} isOpen={isOpen} onTransitionEnd={handleTransitionEnd}>
        <div ref={measuredRef}>
        {children({ setIsOpen, isOpen, setHeight, height })}
      </div>
    </ExpandableContainer>
  )
}
