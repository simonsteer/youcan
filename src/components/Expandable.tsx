...
export interface ExpandableProps {
  children: (renderProps: { setIsOpen: Dispatch<SetStateAction<boolean>>, toggleIsOpen: () => void; isOpen: boolean; setHeight: Dispatch<SetStateAction<number>>, height: number }) => React.ReactNode
  closedHeight?: number
  closeOnBlur?: boolean
  startOpen?: boolean
}

const Expandable = ({ children, closedHeight = 0, startOpen = false, closeOnBlur = false }: ExpandableProps) => {
  ...
  return (
    <ExpandableContainer ref={root} hasOpened={hasOpened} height={height} closedHeight={closedHeight} isOpen={isOpen} onTransitionEnd={handleTransitionEnd}>
      <div ref={measuredRef}>
        {children({ setIsOpen, isOpen, toggleIsOpen: () => setIsOpen(!isOpen), setHeight, height })}
      </div>
    </ExpandableContainer>
  )
}
